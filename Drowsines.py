...
def startMonitoring():
    ...
    svm_predictor = dlib.shape_predictor(args['shape_predictor'])
    (lStart, lEnd) = face_utils.FACIAL_LANDMARKS_IDXS["left_eye"]
    (rStart, rEnd) = face_utils.FACIAL_LANDMARKS_IDXS["right_eye"]
    (mStart, mEnd) = face_utils.FACIAL_LANDMARKS_IDXS["mouth"]
    webcamera = VideoStream(src=args["webcam"]).start()
    time.sleep(1.0)
    while True:
        ...
        for rect in rects:
            ...
            if mouEAR > MOU_AR_THRESH:
                if COUNTER >= MOU_AR_CONSEC_FRAMES:
                    if not ALARM_ON:
                        ALARM_ON = True
                        if args["alarm"] != "":
                            t = Thread(target=sound_alarm,args=(args["alarm"],))
